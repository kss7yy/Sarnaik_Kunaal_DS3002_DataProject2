...
def get_help_response():
    final_response = "Hi! I am the AirVisualBot, and I am here to respond to your queries about temperature and air quality information in the world.\n\nFor a list of my active commands and other helpful information, check out my GitHub ReadMe.md file here: "
    return final_response
...
def check_mentions(api, since_id):
    ...
    for tweet in tweepy.Cursor(api.mentions_timeline,
        ...
        if len(response) <= 280:
            ...
            api.update_status(
                status="@" + tweet.user.screen_name + " Responding to your query via direct message!",
                in_reply_to_status_id=tweet.id
            )
            try:
                api.send_direct_message(tweet.user.id, final_response)
            except Exception as e:
                logger.error("Error! Cannot send direct messages to this user!", exc_info=True)
                raise e
